# This code is adapted from github.com/mkarppa/deann-experiments, which is
# in turn adapted from github.com/erikbern/ann-benchmarks.
...
DATA_DIRECTORY = os.path.join(os.pardir. os.pardir, 'data')
...
def get_dataset_fn(dataset):
    ...
    return os.path.join(DATA_DIRECTORY, '%s.hdf5' % dataset)
...
def write_output(train, validation, test, fn, kernel, compute_bandwidth=False):
...
def covtype(out_fn, compute_bw):
    ...
    fn = os.path.join(DATA_DIRECTORY, 'covtype.gz')
    download(url, fn)
    X = []
...
def census(out_fn, compute_bw, compute_nn):
    ...
    fn = os.path.join(DATA_DIRECTORY, 'census.txt')
    download(url, fn)
    X = []
    with open(fn) as f:
        # skip headerline
...
def shuttle(out_fn, compute_bw):
    ...
    for dn in ("shuttle.trn.Z", "shuttle.tst"):
        ...
        fn = os.path.join(DATA_DIRECTORY, dn)
        download(url, fn)
        if fn.endswith(".Z"):
...
def glove(out_fn, compute_bw, d=100):
    ...
    fn = os.path.join(DATA_DIRECTORY, 'glove.twitter.27B.zip')
    download(url, fn)
...
def sift(out_fn, compute_bw):
    ...
    fn = os.path.join(DATA_DIRECTORY, 'sift.tar.tz')
    download(url, fn)
...
def svhn(out_fn, compute_bw, version):
    ...
    fn = os.path.join(DATA_DIRECTORY, 'svhn-%s.mat' % version)
    download(url, fn)
    X = loadmat(fn)['X']
    d = np.prod(X.shape[:3])
...
def mnist(out_fn, compute_bw):
	...
	os.path.join(DATA_DIRECTORY, 'mnist-train.gz'))
...
def fashion_mnist(out_fn, compute_bw):
    download('http://fashion-mnist.s3-website.eu-central-1.amazonaws.com/train-images-idx3-ubyte.gz',  # noqa
             os.path.join(DATA_DIRECTORY, 'fashion-mnist-train.gz'))
...
def create_dataset(dataset, kernel, compute_bandwidth=True):
    ...
    output_filename = f'{DATA_DIRECTORY}/{filename_prefix}.hdf5'
...
def main():
    ...
    # Pass true as the final argument - this means that we will always compute
    # the bandwidths for the datasets.
    create_dataset(args.dataset, args.kernel, True)
