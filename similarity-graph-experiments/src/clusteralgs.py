...
def cluster_stag_graph(g: stag.graph.Graph, k):
    lap_mat = g.laplacian().to_scipy()
...
def stag_asg_spectral_cluster(dataset: datasets.Dataset, k: int, a=1.0):
    g = stag.cluster.approximate_similarity_graph(
        stag.utility.DenseMat(dataset.raw_data), a)
    return cluster_stag_graph(g, k)
